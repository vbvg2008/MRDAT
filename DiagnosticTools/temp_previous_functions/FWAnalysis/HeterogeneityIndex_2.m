...
for i=1:num_cases
    ...
    Wp(:, i) = case_data{i,1}.Tvar.Field.WaterProductionCumulative.data;
...
AvgNp = sum(Qo, 2, 'omitnan')./num_cases;
AvgWp = sum(Qw, 2, 'omitnan')./num_cases;
...
for i =1:num_cases
    ...
    HI_Np(:, i) = (Np(:, i)./AvgNp) - 1;
    HI_Wp(:, i) = (Wp(:, i)./AvgWp) - 1;
...
% % % 
% % % figure;
% % % scatter(HI_WOR_cum(TotalDaysIdx,:), HI_RF_cum(TotalDaysIdx,:), 20, 'b', 'filled');
% % % xlabel('HI.WOR');
% % % ylabel('HI.RF');
% % % grid on;
...
figure;
gscatter(HI_Np(TotalDaysIdx,:), HI_Wp(TotalDaysIdx,:), ChanPlotFlag, c, '.', 20);
xlabel('HI.Np');
ylabel('HI.Wp');
legend('show');
grid on;
