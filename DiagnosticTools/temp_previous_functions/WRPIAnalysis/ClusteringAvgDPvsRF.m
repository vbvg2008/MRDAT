function case_data = ClusteringAvgDPvsRF(case_data)
% Clustering of average pressure drop in the first 50 or 100 days of
% production vs Final Recovery
%
...
%% Clustering Analysis
 ...
 D = [dPdt50, log10(FinalRF)];  
% D = [dPdt100, FinalRF]; 
% D = [dPdNp50, log10(FinalRF)];
% D = [dPdNp100, log10(FinalRF)];
% D = [dPdRVCP50, log10(FinalRF)];
% D = [dPdRVCP100, log10(FinalRF)];
...
for i=1:num_cases
    case_data{i,1}.Diagnostics.Clustering.AvgDPvsRF = ClusterGroup(i);
end

% Scatter plot by cluster group
figure; 
scatter(D(:,1),D(:,2),[],ClusterGroup, 'filled');
xlabel('Averga Pressure Drop');
