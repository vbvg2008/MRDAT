function case_data = FWCResVolMinMax(varargin)
%Determines gradient of WC w.r.t. reservoir volume production cumulative
...
%
%DESCRIPTION:
%   This function determines gradient of WC w.r.t. reservoir volume 
...
for i = 1:num_FWCcase
    ...
    eval(['case_data{i}.Diagnostics.FPA.Field','.WC.dFWCdRVPC = dydx;']);
    eval(['case_data{i}.Diagnostics.FPA.Field','.WC.dFWCdRVPCmin = dydxmin;']);
    eval(['case_data{i}.Diagnostics.FPA.Field','.WC.dFWCdRVPCminID = idydxmin;']);
    eval(['case_data{i}.Diagnostics.FPA.Field','.WC.dFWCdRVPCminRVPC = xmin;']);    
    eval(['case_data{i}.Diagnostics.FPA.Field','.WC.dFWCdRVPCmax = dydxmax;']);
    eval(['case_data{i}.Diagnostics.FPA.Field','.WC.dFWCdRVPCmaxID = idydxmax;']);
    eval(['case_data{i}.Diagnostics.FPA.Field','.WC.dFWCdRVPCmaxRVPC = xmax;']); 
...
% Handle response
    if strcmp(choice,'Yes')==1
        Frame_data=save_FWCplot(FWC_case_idx,case_data);
    end
end
...
function Frame_data=save_FWCplot(FWC_case_idx,case_data)  %generate plots and frame data
if ~exist('FWCplot','dir')
    mkdir('FWCplot');
end
cd 'FWCplot';
